...
WHERE {
    ...
    OPTIONAL { ?s dpv:isApplicableFor ?applicable_for . }
    OPTIONAL { ?s ftr:supportedBy ?supported_by . } 
    OPTIONAL { ?indimension rdfs:label ?label_dimension . }
    OPTIONAL { ?indimension dcterms:description ?desc_indimension . }
    OPTIONAL { ?benchmark a ftr:Benchmark ;
        dcterms:title ?bm_title;
        dcterms:description ?bm_desc .
    }
    OPTIONAL { ?test a ftr:Test . }
    OPTIONAL { ?s dcterms:creator ?creator_orcid . }
    OPTIONAL { ?creator_orcid vcard:fn ?creator_name . }
    OPTIONAL { ?s dcat:contactPoint ?contact_orcid . }
    OPTIONAL { ?contact_orcid vcard:fn ?contact_name . }
...
def catalog_process(path_mustache_catalog, path_mustache_index, path_source):
    ...
    rendered_index = renderer.render(
        template_content_index, {
                           ...
                           'number_benchmark': f"{len(benchmarks_sorted)} Benchmarks"
                            }
                        )
